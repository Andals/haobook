...
hash分区主要用来把数据均衡地分布于预定义好的分区里。在range和list分区里，需要显式指定一个或者一系列列值保存在哪个分区，而在hash分区里，这些都由MYSQL来决定，开发者只需要指定一个列值或是基于列值的表达式用于hash分区及想要把表分成多少个分区的数值。
...
如果没有包含PARTITIONS子句，默认表分区数为1。也可以基于雇用员工日期作为hash分区，例如：
